...
describe("AMM", function () {
  ...
  describe("provide", function() {
    it("Token should be moved", async function() {
      ...
      // 今回使用する2つのトークンはETHと同じ単位を使用しているので、100 ether (= 100 * 10^18)分をprovideするという意味
      ...
      expect(await token0.balanceOf(owner.address)).to.eql(ownerBalance0Before.sub(amountProvide0));
      expect(await token1.balanceOf(owner.address)).to.eql(ownerBalance1Before.sub(amountProvide1));
  
      expect(await token0.balanceOf(amm.address)).to.eql(ammBalance0Before.add(amountProvide0));
      expect(await token1.balanceOf(amm.address)).to.eql(ammBalance1Before.add(amountProvide1));
    });
  });
  ...
  // deployContractWithLiquidity後の初期値のチェックをする
  describe("Deploy with liquidity", function () {
    it("Should set the right number of amm details", async function () {
      ...
      // ownerのシェア: 最初の流動性提供者なので100に設定
      const BN100 = BigNumber.from("100");
      // otherAccountのシェア: ownerに比べて10分の1だけ提供しているので10に設定
      ...
      expect(await amm.totalAmount(token0.address)).to.equal(amountOwnerProvided0.add(amountOtherProvided0));
      expect(await amm.totalAmount(token1.address)).to.equal(amountOwnerProvided1.add(amountOtherProvided1));
    });
  });
  ...
  describe("getEquivalentToken", function () {
    it("Should get the right number of equivalent token", async function () {
      ...
      expect(await amm.getEquivalentToken(token0.address, amountProvide0)).to.equal(equivalentToken1);
    });
  });
  ...
  describe("getWithdrawEstimate", function () {
    it("Should get the right number of estimated amount", async function () {
      ...
      expect(await amm.getWithdrawEstimate(token0.address, share)).to.eql(amountOtherProvided0);
      expect(await amm.getWithdrawEstimate(token1.address, share)).to.eql(amountOtherProvided1);
    });
  });
  ...
  describe("withdraw", function () {
    it("Token should be moved", async function () {
      ...
      expect(await token0.balanceOf(owner.address)).to.eql(ownerBalance0Before.add(amountOwnerProvided0));
      expect(await token1.balanceOf(owner.address)).to.eql(ownerBalance1Before.add(amountOwnerProvided1));
      
      expect(await token0.balanceOf(amm.address)).to.eql(ammBalance0Before.sub(amountOwnerProvided0));
      expect(await token1.balanceOf(amm.address)).to.eql(ammBalance1Before.sub(amountOwnerProvided1));
    });
    ...
    it("Should set the right number of amm details", async function () {
      ...
      expect(await amm.totalAmount(token0.address)).to.equal(amountOwnerProvided0);
      expect(await amm.totalAmount(token1.address)).to.equal(amountOwnerProvided1);
    });
  });
  ...
  describe("getSwapEstimateOut", function () {
    it("Should get the right number of token", async function () {
      ...
      expect(await amm.getSwapEstimateOut(token0.address, amountInToken0)).to.eql(amountReceiveToken1);
    });
  });
  ...
  describe("getSwapEstimateIn", function () {
    it("Should get the right number of token", async function () {
      ...
      expect(await amm.getSwapEstimateIn(token1.address, amountOutToken1)).to.eql(amountInToken0);
    });

    it("Should revert if the amount of out token exceed the total", async function () {
      const { amm, token1, amountOwnerProvided1, amountOtherProvided1 } = await loadFixture(deployContractWithLiquidity);
  ...
  describe("swap", function () {
    it("Should set the right number of amm details", async function () {
      ...
      const amountReceiveToknen1 = await amm.getSwapEstimateOut(
        token0.address,
        amountSendToken0
      );
      ...
      expect(await amm.totalAmount(token0.address)).to.eql(
        amountOwnerProvided0.add(amountOtherProvided0).add(amountSendToken0)
      );
      expect(await amm.totalAmount(token1.address)).to.eql(
        amountOwnerProvided1.add(amountOtherProvided1).sub(amountReceiveToknen1)
      );
    });
