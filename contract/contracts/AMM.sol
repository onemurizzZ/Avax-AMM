...
contract AMM {
    ...
    // プールに流動生があり、使用可能であることを確認する
    modifier activePool() {
        require(totalShare > 0, "Zero Liquidity");
        _;
    }

    // スマートコントラクトが扱えるトークンであることを確認する
    ...
    modifier validTokens(IERC20 _tokenX, IERC20 _tokenY) {
        ...
        require(_tokenX != _tokenY, "Token should be different!");
        _;
    }

    // 引数のトークンとペアのトークンのコントラクトを返す
    ...
    function swap(
        ...
        external
        activePool
        validTokens(_inToken, _outToken)
        returns(uint256)
    {
